...
class ControllerTest {
    ...
    fun updateUserTest() {
        ...
        mockMvc.perform(post("/update").param("id",lastUser.id.toString()).param("name","updated!!")).andExpect(status().isOk).andExpect(
    ...
    fun deleteNoUserTest() {
        mockMvc.perform(
            post("/delete").param("id", "-1")
        ).andExpect(status().isOk).andExpect(content().string("Non User"))
    }
    ...
    fun deleteUserTest() {
        ...
        mockMvc.perform(
            post("/delete").param("id", lastUser.id.toString())
